#!/usr/bin/env python3
"""
Model 4: NLP Classification — Review-Sentiment BERT + Metadata
===============================================================
Same architecture as train4.py but uses a model pre-trained on product/service
reviews (nlptown/bert-base-multilingual-uncased-sentiment) rather than biomedical
literature (BioBERT).  The intuition: patient medication reviews are closer in
style to Amazon/Yelp reviews than to PubMed abstracts, so sentiment-aware
representations should transfer better.

Pre-training corpus: Amazon, Yelp, TripAdvisor reviews (6 languages, 1-5 star).
Architecture (768-dim hidden, same as BERT-base):

    review text  → SentimentBERT → [CLS] token (768-dim) ─┐
...
Improvements:
    ...
    - OneCycleLR: 30% linear warmup then cosine decay
    - Differential LRs: 2e-5 for transformer, 1e-3 for new layers
...
if not torch.cuda.is_available():
    raise RuntimeError("CUDA not available — check that your GPU drivers and torch+cuda are installed correctly.")
...
class SentimentBERTMetadataClassifier(nn.Module):
    def __init__(self, num_drugs, num_conditions,
        ...
        # 768 (BERT-base hidden size) + 32 + 32
...
def build_meta_encoders(df):
    """Reuse drug/condition encoders from train3.py if available."""
...
def train_model(train_loader, val_loader, num_drugs, num_conditions,
                y_encoded_train, epochs=6, patience=3):
    model = SentimentBERTMetadataClassifier(
        num_drugs=num_drugs,
        num_conditions=num_conditions,
    ).to(DEVICE)

    print("Full BERT fine-tuning — all 12 layers trainable.")
    ...
    print("Sentiment BERT training complete.")
    return model
...
def evaluate_model(model, input_ids, attention_masks, X_drug, X_cond, y_true, target_le, texts=None):
    ...
    plt.title("Confusion Matrix — Sentiment BERT + Metadata", fontsize=13)
    plt.ylabel("Actual", fontsize=12)
    plt.xlabel("Predicted", fontsize=12)
    plt.tight_layout()
    out = SAVED_MODEL_DIR / "confusion_matrix_train6.png"
...
def save_artifacts(model, target_le):
    SAVED_MODEL_DIR.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), SAVED_MODEL_DIR / "model_sentiment_bert_meta.pt")
    joblib.dump(target_le, SAVED_MODEL_DIR / "label_encoder_sentiment_bert.joblib")
    print(f"Saved Sentiment BERT model to {SAVED_MODEL_DIR}")
...
def main():
    ...
    # 6. Train
    print(f"\n=== Training Sentiment BERT + Metadata on {len(idx_train):,} reviews ===")
